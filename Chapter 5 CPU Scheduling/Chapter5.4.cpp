    ...
    void scheduler()
    {
        while (running || active_threads > 0)
        {
            ...
            if (!ready_queue.empty())
            {
                ...
                auto turnaround_time = std::chrono::duration_cast<std::chrono::milliseconds>(current_thread.completion_time - current_thread.arrival_time);
...
public:
    ...
    void displayAttributes()
    {
        ...
        std::cout << "  Policy: " << (policy == SCHED_FIFO ? "FIFO" : policy == SCHED_RR ? "Round Robin"
                                                                                         : "Other")
                  << "\n";
